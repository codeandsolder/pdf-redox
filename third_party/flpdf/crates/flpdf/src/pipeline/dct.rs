...
impl<'a> PlDct<'a> {
    ...
    pub(crate) fn new_compressor(
        ...
        Self::new_compressor_with_quality(identifier, next, width, height, pixel_format, 75)
    }

    pub(crate) fn new_compressor_with_quality(
        identifier: impl Into<String>,
        next: impl Into<PipelineRef<'a>>,
        width: usize,
        height: usize,
...
mod tests {
    ...
    #[test]
    fn compressor_quality_changes_encoded_output() {
        let pixels: Vec<u8> = (0..64 * 64)
            .map(|index| ((index * 37 + (index / 64) * 19) % 256) as u8)
            .collect();

        let encode = |quality| {
            let trace = shared_trace();
            let mut sink = RecordingSink::with_trace(trace.clone(), &[], &[]);
            let mut stage = PlDct::new_compressor_with_quality(
                "jpg",
                &mut sink,
                64,
                64,
            ...
            stage.write(&pixels).expect("compressor accepts image");
            stage.finish().expect("compressor finishes");
            drop(stage);
        ...
        let low_quality = encode(30);
        let high_quality = encode(95);
